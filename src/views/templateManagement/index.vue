	...
	<div class="container">
		...
		<el-form :inline="true" :model="searchForm" class="search-form">
			<div class="collapse_show_item">
				<el-form-item label="模板名称:" label-width="80px">
					<el-input v-model="searchForm.name" placeholder="方案名称" style="width: 200px"></el-input>
				</el-form-item>
				<el-form-item label="报告模板分类:" label-width="100px" v-if="searchForm.type === 2">
          <el-select v-model="searchForm.reportType" clearable placeholder="请选择报告模板分类" style="width: 200px">
            <el-option
		...
		<el-card>
			<div class="tabs-header-wrapper">
				<el-button type="primary" class="extra-button" @click="onAdd">{{searchForm.type === 1 ? '新增测试方案模板' : '新增测试报告模板'}}</el-button>
						...
						<div>
							<el-table v-loading="loading"
								...
								<el-table-column
									...
									align='center'>
									<template #default="scope">
										<el-select :model-value="scope.row.status" @change="(value) => handleUpdate(value, scope.row, 'status')">
						...
						<div>
							<el-table v-loading="loading"
								...
								<el-table-column
									...
									align='center'>
									<template #default="scope">
										{{ scope.row.report_type === 1 ? '出厂/验收报告': scope.row.is_type === 2 ? '入网安评' : '源码审计' }}
									</template>
								</el-table-column>		
								<!-- <el-table-column
									prop="is_default"
		...
		>
			<el-form label-width="120px" :model="form" :rules="formRules" ref="formRef" v-loading="saveLoading" element-loading-text="新增模板中...">
              ...
              :key="item.value"
				...
				<el-form-item label="报告模板归类：" prop="reportType" required v-if="form.businessType === 2">
              ...
              :key="item.value"
				...
				<el-form-item label="文档模板：" prop="modelFile" required>
					<el-upload
						...
						accept=".docx"
						:on-change="(uploadFile, uploadFiles) => handleFileChange(uploadFile, uploadFiles, uploadConfig.modelFile)"
...
import { getDocumentList, uploadTemplateFile, addTemplate, updateTemplate } from '@/api/templateManagement';
...
const handleClose = async () => {
  if (modelLoading.value || saveLoading.value) {
		const message = modelLoading.value ? '文档模板正在上传中，确定要关闭吗？' : '新增模板中，确定要关闭吗？'
		try {
