  ...
  <div class="container">
    ...
    <div>
      ...
      >
        ...
        <el-table-column prop="name" label="方案名称" min-width="140" align="center">
        </el-table-column>
        <el-table-column prop="radio" label="是否涉及移动端" width="110" align="center">
    ...
    >
      ...
      >
        <div class="tips-contant">
          ...
          <p class="highlight">
            3、需规存在多分册情况下，请注意手动将文档名称调整为“XXX说明书-XX分册”，分册名放在横行“-”最后！
          </p>
          <p>
            4、性能清单表：如《委托单》文档中的《测试需求》没涉及性能，可以不上传，反之，必传！！
          </p>
          <p>
            5、Excel表格中《表头》需保持一致，案例模板：性能清单表：
            <el-button text type="primary" @click="downloadTemplate(1)">性能清单表模板</el-button>
            资产收集表：<el-button
              text
              type="primary"
          ...
          <p class="highlight">
            6、如果《安全用例范围》有变动，则需要上传更改模板：安全范围：<el-button
              text
              type="primary"
          ...
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.commissionFile)
            "
        ...
        <el-form-item label="需求规格说明书：" prop="demandFile" required>
          ...
          <div v-if="formFile.demand_file.length" class="uploaded-file-info">
            <div
              class="file-card"
              v-for="(file, index) in formFile.demand_file"
              :key="index"
        ...
        <el-form-item label="性能清单表：" prop="performanceFile" required>
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.performanceFile)
            "
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click="handleGenerate"> 生成测试方案 </el-button>
        </div>
      </template>
    </el-dialog>
    ...
    >
      <el-form label-width="102px" :model="form" ref="personnelFormRef">
        <el-tag type="info" size="large" style="margin-bottom: 20px; font-size: 14px"
          >温馨提示：人员名称填写，请注意用、隔开，例如：张三、李四</el-tag
        >
  ...
  Promise.allSettled(uploadPromises)
    .then(() => {
      if (successAttachmentIds.length > 0) {
        ...
        ElMessage.success(
          `成功上传 ${successAttachmentIds.length} 个文件，失败 ${validFiles.length - successAttachmentIds.length} 个`,
        )
      }
    })
    .finally(() => {
