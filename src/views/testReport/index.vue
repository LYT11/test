  ...
  <div class="container">
      ...
      <div class="collapse_show_item">
        ...
        <el-form-item label="状态:" label-width="50px">
          <el-select
            ...
            clearable
            placeholder="请选择状态"
            style="min-width: 120px; width: 180px"
          >
            <el-option
        ...
        <!-- <el-form-item label="报告模板分类:" label-width="100px">
          <el-select v-model="searchForm.type" clearable placeholder="请选择报告模板分类" style="min-width: 120px; width: 180px">
            <el-option
    ...
    <div>
      ...
      >
        ...
        <el-table-column prop="name" label="报告名称" min-width="140" align="center">
        </el-table-column>
        <el-table-column prop="is_type" label="报告模板分类" width="120" align="center">
        ...
        <el-table-column prop="status" label="状态" width="100" align="center">
          <template #default="scope">
            <el-tag size="small" :type="STATUS_OPTIONS[+(scope.row.status - 1)].type">
    ...
    >
      <el-form
        ...
        element-loading-text="生成测试报告中..."
      >
        <div class="tips-contant">
          <p class="highlight">温馨提示：</p>
          <p class="highlight">
            1、如文档格式不一致，请手动用WPS打开文档，另存为对应的文档格式！如：.doc转.docx
            xls转.xlsx
          </p>
          <p v-if="form.type === 1">
            2、《委托单》《测试方案》必传！缺一不可！《功能/性能/安全测试数据汇总》三者至少要传其一（上传哪个数据就生成哪部分报告）！
          </p>
          <p class="highlight" v-if="form.type !== 3">
            {{ form.type === 2 ? '2、' : '3、' }}测试数据汇总表格中《表头》需保持一致，模板下载：
          </p>
          <p v-if="form.type === 1">
            功能测试数据汇总表：
            <el-button text type="primary" @click="downloadTemplate(1)"
              >功能测试数据汇总表模板</el-button
            >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;性能测试数据汇总表：<el-button
              text
              type="primary"
              @click="downloadTemplate(2)"
              style="margin-left: 0"
              >性能测试数据汇总表模板</el-button
            >
          </p>
          <p v-if="form.type === 1">
            安全测试数据汇总表：
            <el-button text type="primary" @click="downloadTemplate(3)">
              安全测试数据汇总表模板
            </el-button>
          </p>
          <p v-if="form.type === 2">
            测试记录文档：
            <el-button text type="primary" @click="downloadTemplate(5)"
              >入网安评测试记录模板</el-button
            >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;问题单文档：<el-button
              text
              type="primary"
              @click="downloadTemplate(4)"
              style="margin-left: 0"
              >入网安评测试问题集模板</el-button
            >
          </p>
        </div>
          ...
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.commission)
            "
        ...
        <el-form-item label="测试方案文档：" prop="plan" required v-if="form.type === 1">
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.plan)
            "
        ...
        >
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.functionCollect)
            "
            :disabled="functionCollectLoading"
          >
            <el-icon class="upload-icon"><UploadFilled /></el-icon>
            <div class="upload-text">将文件拖到此处，或 <em>点击上传</em></div>
            <div class="upload-hint">仅支持格式：.xlsx，文件总大小不超过 {{ maxSize }}MB</div>
          </el-upload>
            ...
            <div class="file-card">
              ...
              <div class="file-details">
                ...
                <span class="file-size">{{
                  formatFileSize(formFile.functionCollect_file.size)
                }}</span>
              </div>
              <div class="file-actions">
        ...
        >
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.performanceCollect)
            "
          ...
          <div v-if="formFile.performanceCollect_file" class="uploaded-file-info">
            <div class="file-card">
              ...
              <div class="file-details">
                ...
                <span class="file-size">{{
                  formatFileSize(formFile.performanceCollect_file.size)
                }}</span>
              </div>
              <div class="file-actions">
        ...
        >
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.secureCollect)
            "
          ...
          <div v-if="formFile.secureCollect_file" class="uploaded-file-info">
            <div class="file-card">
              ...
              <div class="file-details">
                ...
                <span class="file-size">
                  {{ formatFileSize(formFile.secureCollect_file.size) }}
                </span>
              </div>
              <div class="file-actions">
        ...
        <el-form-item label="测试记录文档：" prop="testRecord" required v-if="form.type === 2">
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.testRecord)
            "
        ...
        >
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.troubleTicket)
            "
          ...
          >
            ...
            <div class="upload-hint">
              仅支持格式：{{ form.type === 2 ? '.xlsx' : '.docx' }}，文件总大小不超过
              {{ maxSize }}MB
            </div>
          </el-upload>
          ...
          <div v-if="formFile.troubleTicket_file" class="uploaded-file-info">
            <div class="file-card">
              ...
              <div class="file-details">
                ...
                <span class="file-size">{{
                  formatFileSize(formFile.troubleTicket_file.size)
                }}</span>
              </div>
              <div class="file-actions">
        ...
        <el-form-item label="资产收集表：" prop="assetFile" required v-if="form.type === 2">
          <el-upload
            ...
            :on-change="
              (uploadFile, uploadFiles) =>
                handleFileChange(uploadFile, uploadFiles, uploadConfig.assetFile)
            "
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="handleGenerate"> 生成测试报告 </el-button>
        </div>
      </template>
    </el-dialog>
...
import { STATUS_OPTIONS, REPORT_TYPE_OPTIONS } from '@/constants/common'
...
import { uploadFile, deleteFile, downloadFile } from '@/api/file'
...
const getDataList = async () => {
  try {
    ...
    if (res.code === 200) {
      res.data.map((item) => {
        item.statusText = item.status ? STATUS_OPTIONS[+(item.status - 1)].label : ''
      })
